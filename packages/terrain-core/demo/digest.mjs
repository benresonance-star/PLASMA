// Synchronous SHA-256 for small bounded operation envelopes on the reflex path.
// Source and mesh digests use Web Crypto outside that path.
...
export function operationDigest(text) {
  const input = new TextEncoder().encode(text);
  if (input.length > 8192) throw Error("Operation digest byte budget exceeded.");
  ...
  return "sha256:" + h.map(v => (v>>>0).toString(16).padStart(8,"0")).join("");
}
