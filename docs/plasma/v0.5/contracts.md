...
## Durable computation provenance — PLS-15 / PLS-RUN-01
[RunRecord semantics](run-records.md) and [candidate types](run-records.ts) require permanently addressable terminal computation records. Runs retain pinned inputs, producer/version, assumptions, parameters and artifact/evidence references outside semantic World State. Completion does not publish. Accepted revision events bind the proposal and run atomically; currentness and publication links are derived views. CON-031 defines the restart, historical inspection, rerun and recovery-copy acceptance test.
