...
/**
 * Deterministic exact-geometry adapter behind the OCCT service boundary.
 * Binding choice (ADR): Docker-hosted service; CI uses this adapter until
 * OCCT WASM worker image is provisioned. Same DTOs / validation states.
 */
export class ExactKernelAdapter {
  ...
  tessellate(req: TessellateRequest): Mesh {
    ...
    const maxDeviationMm = Math.min(req.chordDeviationMm, req.angleDeviationDeg * 0.1);
    return { vertices, indices, maxDeviationMm };
  }
