...
function pathMatches(path,patterns=[]){ return patterns.some(pattern=>globToRegExp(pattern).test(path)); }

function bindingPatterns(binding){
  const sourcePatterns=(binding.sources||[]).flatMap(source=>source.paths||[]);
  return [...new Set([...sourcePatterns,'apps/system-atlas-preview/atlas.json','apps/system-atlas-preview/repository-evidence.json'])];
}
...
function cacheWrite(value){ try{ sessionStorage.setItem(CACHE_KEY,JSON.stringify({cachedAt:Date.now(),value})); }catch{} }

function findJob(jobs,jobName){ return (jobs||[]).find(job=>job.name===jobName)||(jobs||[])[0]||null; }
function findStep(job,stepName){ return (job?.steps||[]).find(step=>step.name===stepName)||null; }
...
function observationFromRun({binding,run,job,step,headSha,stale=false,statusOverride=null}){
  ...
  return {
    ...
    repository:{
      provider:'github',headSha,testedCommitSha:run?.head_sha||null,workflow:binding.ci?.workflow||null,
      workflowRunId:run?.id||null,workflowUrl:run?.html_url||null,job:job?.name||binding.ci?.job||null,
      step:step?.name||binding.ci?.step||null,runStatus:run?.status||null,runConclusion:run?.conclusion||null
    }
  };
}
...
async function fetchJobs(repoBase,runs){
  const entries=await Promise.all((runs||[]).slice(0,6).map(async run=>{
    try{ const jobs=await githubJson(repoBase+'/actions/runs/'+run.id+'/jobs?per_page=100'); return [run.id,jobs.jobs||[]]; }
    catch{ return [run.id,[]]; }
  }));
...
export async function loadRepositoryEvidenceStatus(bindingsDoc){
  ...
  if(local){
    return {mode:'offline',available:false,message:'Live repository evidence is disabled on the local smoke-test server.',headSha:null,updatedAt:new Date().toISOString(),bindings:bindingsDoc,observations:{}};
  }
  ...
  try{
    const values=await Promise.all([
      githubJson(repoBase+'/commits/'+encodeURIComponent(branch)),
      githubJson(repoBase+'/actions/workflows/'+encodeURIComponent(bindingsDoc.workflow.file)+'/runs?branch='+encodeURIComponent(branch)+'&event=push&per_page=12')
    ]);
    const head=values[0];
    const runsPayload=values[1];
    const headSha=head.sha;
    const runs=(runsPayload.workflow_runs||[]).filter(run=>run.head_branch===branch);
    ...
    for(const binding of bindingsDoc.bindings||[]){
      if(!binding.ci||(binding.tests||[]).length===0){
        observations[binding.evidenceId]={status:'unlinked',method:'repository-binding',source:null,subjectRevision:null,verifiedAt:null,result:null,bindingId:binding.id,repository:{provider:'github',headSha,testedCommitSha:null,workflow:null,workflowRunId:null}};
        continue;
      }
      ...
      if(currentRun&&currentStep?.status==='completed'&&(currentStep.conclusion==='success'||currentStep.conclusion==='failure')){
        observations[binding.evidenceId]={...observationFromRun({binding,run:currentRun,job:currentJob,step:currentStep,headSha}),bindingId:binding.id};
        continue;
      }
      ...
      if(!previous){
        observations[binding.evidenceId]={status:'linked',method:binding.tests?.[0]?.framework||'ci',source:binding.tests?.flatMap(test=>test.paths||[]).join(', ')||null,subjectRevision:null,verifiedAt:null,result:currentRun?.status||null,bindingId:binding.id,repository:{provider:'github',headSha,testedCommitSha:null,workflow:binding.ci.workflow,workflowRunId:currentRun?.id||null}};
        continue;
      }
      ...
      observations[binding.evidenceId]={
        ...observationFromRun({binding,run:previous.run,job:previous.job,step:previous.step,headSha,stale:relevant}),
        bindingId:binding.id,
        staleReason:relevant?'Relevant source, evidence definition or binding changed since the last passing observation.':null
      };
    }

    const value={mode:'live',available:true,message:'Live evidence read from GitHub Actions for the current main branch.',headSha,updatedAt:new Date().toISOString(),bindings:bindingsDoc,observations};
    cacheWrite(value);
    return value;
  }catch(error){
    return {mode:'degraded',available:false,message:error instanceof Error?error.message:String(error),headSha:null,updatedAt:new Date().toISOString(),bindings:bindingsDoc,observations:{}};
  }
}
